...
本项目为 **中国法研杯司法人工智能挑战赛（CAIL2021） 论辩理解**赛道 任务1.2 **跨案件类型的争议观点对迁移学习** 的基线模型和参考代码，该模型是基于BERT的神经网络模型。
...
本项目中只使用了 `SMP-CAIL2021-train.csv`： 包含了2449对裁判文书中的互动论点对（与任务1.1的训练数据完全一致）。分别包含以下维度：
...
```
python main.py --model_config 'config/bert_config.json' \
               --in_file 'SMP-CAIL2021-crossdomain-test1.csv' \
               --out_file 'bert-submission-crossdomain-test-1.csv'
```

### 3. 评估结果

`golden_file`为带真实答案标签的文件，`predict_file`为待测试模型生成的结果文件。第一阶段仅提供训练集`SMP-CAIL2021-train.csv`的真实答案标签，跨领域学习的测试集`SMP-CAIL2021-crossdomain-test1.csv`的真实答案标签暂不提供。运行结果输出Accuracy和F1分数。
